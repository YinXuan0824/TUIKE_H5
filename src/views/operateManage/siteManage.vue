 ...
 * @LastEditTime: 2025-01-09 14:36:42
 * @Description: 站点管理
-->
...
function handleSave() {
  if (isEdit.value) {
    handleSaveFun(editSiteManage, formData).then(() => {
      closeDialog()
      handleQuery()
    })
  } else {
    handleSaveFun(saveSiteManage, formData).then(() => {
      closeDialog()
      handleQuery()
    })
  }
...
function handleDelete(row: any) {
  handleBatchDeleteFun(deleteSiteManage, [row.id]).then(() => {
    handleQuery()
  })
}
...
function handleEdit(row: any) {
  ...
  formData.siteNo = row.siteNo
  formData.direction = row.direction
  formData.coverageRadius = row.coverageRadius
}
...
function handleQuery() {
  handleQueryFun(fetchSiteManageList, queryParams)
}
...
const rules = {
  ...
  siteSort: [{ required: true, message: '请输入站点排序', trigger: 'blur' }],
  ...
  <div class="app-container">
    ...
    <el-card shadow="never" size="small">
      ...
      <el-table v-loading="loading" :data="menuList" highlight-current-row border size="small">
        <el-table-column label="站点名称" align="center" width="150" prop="siteName" />

        <el-table-column label="站点排序" align="center" width="150" prop="siteSort" />
        ...
        <el-table-column label="站点编号" align="center" width="150" prop="siteNo" />

        <el-table-column label="方向" align="center" width="150" prop="direction" />

        <el-table-column label="覆盖面积(米)" align="center" width="150" prop="coverageRadius" />
        ...
        <div>
          <el-form ref="menuFormRef" label-position="top" label-width="100px" :model="formData" :rules="rules">
            ...
            <el-form-item label="站点编号" prop="siteNo">
              <el-input v-model="formData.siteNo" clearable />
            </el-form-item>
            <el-form-item label="方向" prop="direction">
              <el-input v-model="formData.direction" clearable />
            </el-form-item>
            <el-form-item label="覆盖面积(米)" prop="coverageRadius">
              <el-input v-model="formData.coverageRadius" clearable />
            </el-form-item>
          </el-form>
        </div>
      </template>
